...
rule all:
    ...
    input:
        ...
        expand(output_folder+"/07.stats/{chr}/CHUNKS/{chr}_{g_chunk}_{ext}",chr=chrs,ext=['impute_summary_by_maf_by_info.csv','impute_summary_by_maf.csv','impute_summary.pdf','impute_manhattan.png'],g_chunk=["{:02d}".format(chunk) for chunk in list(range(1,getChunkNumByChr+1))])
