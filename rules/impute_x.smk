rule CreateSexLists:
	output:
		females=output_folder + "/00.cleaned_input/" + cohort_name + "_females.list"
		males=output_folder + "/00.cleaned_input/" + cohort_name + "_males.list"
	input:
		input_prefix + ".ped"
	params:
	...
	reasources:
		mem_mb=2000
	log:
		stdout=log_folder+"/CreateSexLists.o",	
		stderr=log_folder+"/CreateSexLists.e"
	shell:
		"""
		awk '$5 == 1 {print $1} {input} > {output.males}'
		awk '$5 == 2 {print $1} {input} > {output.females}'
		"""

# rule to split females and males
rule SplitSexSamples:
	output:
	input:
	params:
	threads: 1
	reasources:
		mem_mb=2000
	log:
		stdout=log_folder+"/SplitSexSamples_{chr}.o",	
		stderr=log_folder+"/SplitSexSamples_{chr}.e"
	shell:
		"""
		"""
